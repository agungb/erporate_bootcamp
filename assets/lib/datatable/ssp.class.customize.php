...
class SSP {
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    ...
                    /*$temp= explode(" AS ",  $column['db']);
                    $column['db']=  current($temp);
                    if(substr($column['db'],0,3)=='SUM'){
                        $column['db'] = substr($column['db'],4,-1);
                    }else if(substr($column['db'],0,5)=='COUNT'){
                        $column['db'] = substr($column['db'],6,-1);
                    }else if(substr($column['db'],0,7)=='AVERAGE'){
                        $column['db'] = substr($column['db'],8,-1);
                    }*/
     ...
     *
     */
    static function complex ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '')
    {
        $bindings = array();
